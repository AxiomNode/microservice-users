...
const ConfigSchema = z
    ...
    const hasCredentialTriplet = Boolean(config.FIREBASE_PROJECT_ID && config.FIREBASE_CLIENT_EMAIL && config.FIREBASE_PRIVATE_KEY);
    if (!hasCredentialsJson && !hasCredentialTriplet) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            path: ["FIREBASE_STRICT_AUTH"],
            message: "Strict Firebase auth requires FIREBASE_CREDENTIALS_JSON or FIREBASE_PROJECT_ID/FIREBASE_CLIENT_EMAIL/FIREBASE_PRIVATE_KEY"
        });
        return;
    }
    if (hasCredentialsJson) {
        try {
            const parsed = JSON.parse(config.FIREBASE_CREDENTIALS_JSON);
            if (!parsed.project_id || !parsed.client_email || !parsed.private_key) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    path: ["FIREBASE_CREDENTIALS_JSON"],
                    message: "FIREBASE_CREDENTIALS_JSON must include project_id, client_email and private_key"
                });
            }
        }
        catch {
