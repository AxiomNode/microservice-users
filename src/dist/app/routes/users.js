...
export async function usersRoutes(app, firebaseAuth, userService, metrics) {
    async function requireAuth(request, reply) {
        const req = request;
        try {
            const identity = await firebaseAuth.authenticateFromBearer(request.headers.authorization, typeof request.headers["x-dev-firebase-uid"] === "string"
    ...
    app.post("/users/firebase/session", async (request, reply) => {
        ...
        try {
            ...
            const identity = await firebaseAuth.authenticateFromBearer(idToken ? `Bearer ${idToken}` : request.headers.authorization, typeof request.headers["x-dev-firebase-uid"] === "string"
                ? request.headers["x-dev-firebase-uid"]
                : undefined);
            metrics.recordAuthAttempt(true);
    ...
    app.patch("/users/admin/roles/:firebaseUid", { preHandler: requireAuth }, async (request, reply) => {
        ...
        try {
            const updated = await userService.updateUserRoleByFirebaseUid(identity.firebaseUid, params.data.firebaseUid, parsedBody.data.role);
            return reply.send({
