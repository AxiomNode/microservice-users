export class ServiceMetrics {
    ...
    usersCreatedTotal = 0;
    usersUpdatedTotal = 0;
    gameEventsStoredTotal = 0;
    gamesWonTotal = 0;
    gamesLostTotal = 0;
    gamesDrawTotal = 0;
    gameEventsByType = new Map();
    gameEventsByLanguage = new Map();
    constructor(config) {
        this.config = config;
    }
    ...
    recordGameEventStored(payload) {
        ...
        this.gameEventsByType.set(payload.gameType, (this.gameEventsByType.get(payload.gameType) ?? 0) + 1);
        this.gameEventsByLanguage.set(payload.language, (this.gameEventsByLanguage.get(payload.language) ?? 0) + 1);
    }
    ...
    toPrometheus() {
        ...
        for (const [gameType, total] of this.gameEventsByType.entries()) {
            lines.push(`microservice_game_events_by_type_total{game_type="${gameType}"} ${total}`);
        }
        for (const [language, total] of this.gameEventsByLanguage.entries()) {
            lines.push(`microservice_game_events_by_language_total{language="${language}"} ${total}`);
        }
        return lines.join("\n");
    }
}
